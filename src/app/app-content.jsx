...
const AppContent = observer(() => {
    ...
    const { connectionStatus, isAuthorizing, isAuthorized } = useApiBase();
    ...
    React.useEffect(() => {
        if (is_api_initialized) {
            init();
            // Check localStorage for auth tokens — the client store may not have
            // processed them yet even though they exist (timing after OAuth callback)
            const hasStoredAuth =
                !!localStorage.getItem('active_loginid') || !!localStorage.getItem('authToken');

            if (!client.is_logged_in && !hasStoredAuth) {
                // Truly not logged in with no pending auth — show landing page immediately
                setIsLoading(false);
            } else {
                // Either already logged in, or tokens exist and auth is still processing
                setIsLoading(true);
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    // Handle transition: user logged in while landing page was showing
    React.useEffect(() => {
        if (client.is_logged_in && is_api_initialized && !is_loading) {
            // Was showing landing page, now authenticated — start the platform
            setIsLoading(true);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    // Break infinite loading loop when auth definitively fails
    const auth_started = React.useRef(false);
    React.useEffect(() => {
        if (isAuthorizing) auth_started.current = true;
        if (
            is_loading &&
            is_api_initialized &&
            !client.is_logged_in &&
            auth_started.current &&
            !isAuthorizing &&
            !isAuthorized
        ) {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [isAuthorizing, isAuthorized, is_loading, is_api_initialized]);

    // Hard 20-second timeout: unblock if still loading with no resolution
    React.useEffect(() => {
        if (!is_loading || !is_api_initialized) return;
        const timeout = setTimeout(() => {
            if (!client.is_logged_in) {
                localStorage.removeItem('authToken');
                localStorage.removeItem('active_loginid');
