...
const CoreStoreProvider: React.FC<{ children: React.ReactNode }> = observer(({ children }) => {
    ...
    useEffect(() => {
        if (!isAuthorizing && isAuthorized && !accountInitialization.current && client) {
            ...
            const fetchLandingCompany = (country_code?: string) => {
                const country = country_code || activeAccount?.landing_company_name || 'svg';
                api_base.api
                    .landingCompany({ landing_company: country })
            ...
            api_base.api
                ...
                .then((settingRes: TSocketResponseData<'get_settings'>) => {
                    if (settingRes?.get_settings) {
                        ...
                        fetchLandingCompany(settingRes.get_settings?.country_code);
                    } else {
                        fetchLandingCompany();
                    }
                })
